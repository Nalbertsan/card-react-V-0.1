    ...
    return (
        <div className="flex w-full h-full items-start justify-center">
            <div className='flex flex-col items-center gap-3'>
                <Input size='md' label='Nome do Titular' value={name} maxLength={20} minLength={3} onChange={(e) => { handleName(e) }} />
                <Input label='Número do Cartão' maxLength={19} value={cardNumber} type='text' containerProps={{ className: "min-w-[72px]" }} onChange={(e) => { handleCardNumber(e) }} />
                <div className="my-1 flex items-center gap-3">
                    <Input label='Validade(MM/AA)' value={formatDate(date)} onChange={(e) => { handleDate(e.target.value) }} maxLength={5} containerProps={{ className: "min-w-[72px]" }} />
